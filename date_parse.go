...
// ParseDateAny parse an unknown date format, detect the layout.
// Normal parse.  Equivalent Timezone rules as time.Parse().
// NOTE:  please see readme on mmdd vs ddmm ambiguous dates.
func ParseDateAny(datestr string, opts ...ParserOption) (time.Time, error) {
...
// ParseDateIn with Location, equivalent to time.ParseInLocation() timezone/offset
...
// in other locations.
func ParseDateIn(datestr string, loc *time.Location, opts ...ParserOption) (time.Time, error) {
...
// ParseDateLocal Given an unknown date format, detect the layout,
...
//
//	t, err := watchmaker.ParseDateLocal("3/1/2014")
//
// Equivalent to:
//
//	t, err := watchmaker.ParseDateLocal("3/1/2014", denverLoc)
func ParseDateLocal(datestr string, opts ...ParserOption) (time.Time, error) {
...
// ParseDateFormat parse's an unknown date-time string and returns a layout
// string that can parse this (and exact same format) other date-time strings.
//
//	layout, err := watchmaker.ParseDateFormat("2013-02-01 00:00:00")
//	// layout = "2006-01-02 15:04:05"
func ParseDateFormat(datestr string, opts ...ParserOption) (string, error) {
	p, err := parseTime(datestr, nil, opts...)
	if err != nil {
		return "", err
	}
...
// ParseDataStrict parse an unknown date format.  IF the date is ambigous
// mm/dd vs dd/mm then return an error. These return errors:   3.3.2014 , 8/8/71 etc
func ParseDataStrict(datestr string, opts ...ParserOption) (time.Time, error) {
	p, err := parseTime(datestr, nil, opts...)
	if err != nil {
		return time.Time{}, err
	}
