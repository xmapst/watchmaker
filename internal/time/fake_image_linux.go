...
import (
	...
	"github.com/xmapst/watchmaker/internal/mapreader"
	"github.com/xmapst/watchmaker/internal/ptrace"
)
...
func (it *FakeImage) AttachToProcess(pid int, variables map[string]uint64) (err error) {
	...
	defer func() {
		err = program.Detach()
		if err != nil {
			logx.Errorln(err, "fail to detach program", "pid", program.Pid())
		}
	}()
	...
	if fakeEntry == nil {
		...
		defer func() {
			if err != nil {
				...
				if errIn != nil {
					logx.Errorln(errIn, "rewrite fail, recover fail")
				}
...
func (it *FakeImage) FindInjectedImage(program *ptrace.TracedProgram, varNum int) (*mapreader.Entry, error) {
	logx.Infoln("finding injected image")
	...
	if it.fakeEntry != nil {
		...
		logx.Infoln("successfully read slice", "content", contentWithoutVariable, "expected content", expectedContentWithoutVariable)

		if bytes.Equal(contentWithoutVariable, expectedContentWithoutVariable) {
			logx.Infoln("slice found")
			return it.fakeEntry, nil
		}
		logx.Infoln("slice not found")
	}
	return nil, nil
}
