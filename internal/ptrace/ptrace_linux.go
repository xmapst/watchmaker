		...
		for _, thread := range threads {
			...
			if err != nil {
				...
				if retryCount[tid] < threadRetryLimit {
					logx.Infoln("retry attaching thread", "tid", tid, "retryCount", retryCount[tid], "limit", threadRetryLimit)
					continue
				}
			...
			defer func() {
				if !traceSuccess {
					...
					if err != nil {
						if !strings.Contains(err.Error(), "no such process") {
							logx.Errorln(err, "detach failed", "tid", tid)
						}
					}
				}
			}()
			...
			logx.Infoln("attach successfully", "tid", tid)
			tids[tid] = true
			tidMap[tid] = true
		}
...
func (p *TracedProgram) Detach() error {
	...
	logx.Infoln("Successfully detach and rerun process")
	return nil
}
