...
import (
	...
	"github.com/xmapst/watchmaker/internal/util"
)
...
func NewProcessGroupHandler(leader ChaosOnProcessGroup) ProcessGroupHandler {
...
func (gp *ProcessGroupHandler) Inject(pid IsID) error {
	...
	childPIDs, err := util.GetChildProcesses(uint32(sysPID))
	if err != nil {
	...
	for _, childPID := range childPIDs {
		...
		if childProcessChaos, ok := gp.childMap[childSysPID]; ok {
			...
			if err != nil {
				logx.Errorln(err, "failed to assign old child process")
				continue
			}
			err = childProcessChaos.Inject(childSysPID)
			if err != nil {
				logx.Errorln(err, "failed to inject old child process")
			}
		} else {
			childProcessChaos, err := gp.LeaderProcess.Fork()
			if err != nil {
				logx.Errorln(err, "failed to create child process")
				continue
			}
			err = childProcessChaos.Inject(childSysPID)
			if err != nil {
				logx.Errorln(err, "failed to inject new child process")
				continue
			}
...
func (gp *ProcessGroupHandler) Recover(pid IsID) error {
	...
	for childID, group := range gp.childMap {
		...
		if !ok {
			logx.Errorln(cerr.NotType[SysPID]().WrapInput(childID).Err(),
				"failed to recover old child process")
		}

		err := group.Recover(childSysPID)
		if err != nil {
			logx.Errorln(err, "failed to recover old child process")
		}
	}
	return nil
}
